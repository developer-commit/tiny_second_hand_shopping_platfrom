...
fn map_escrow_error(e: crate::service::traits::EscrowServiceError) -> AppError {
    match e {
        crate::service::traits::EscrowServiceError::ProductNotFound => {
            AppError::NotFound("Product not found".to_string())
        }
        crate::service::traits::EscrowServiceError::EscrowNotInitiated => {
            AppError::NotFound("에스크로가 아직 시작되지 않았습니다.".to_string())
        }
        crate::service::traits::EscrowServiceError::TradeNotFound => {
            AppError::NotFound("Trade not found".to_string())
        }
        crate::service::traits::EscrowServiceError::InsufficientBalance => {
            AppError::BadRequest("Insufficient balance".to_string())
        }
        crate::service::traits::EscrowServiceError::DuplicateTrade => {
            AppError::BadRequest("Duplicate trade".to_string())
        }
        crate::service::traits::EscrowServiceError::InvalidState(s) => {
            AppError::BadRequest(format!("Invalid state: {}", s))
        }
        crate::service::traits::EscrowServiceError::Forbidden => AppError::Forbidden,
        crate::service::traits::EscrowServiceError::InvalidWalletAddress => {
            AppError::BadRequest("Invalid wallet address".to_string())
        }
        crate::service::traits::EscrowServiceError::ContractRevert(msg) => {
            AppError::BadRequest(format!("Contract revert: {}", msg))
        }
        crate::service::traits::EscrowServiceError::RpcError(msg) => {
            AppError::BadGateway(format!("RPC error: {}", msg))
        }
