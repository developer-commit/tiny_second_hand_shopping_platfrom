...
pub async fn get_wallet(
    ...
    state.wallet_service.get_wallet_state(user_id).await
        .map(Json)
        .map_err(|e| {
...
pub async fn get_tx_history(
    ...
    let user_id = deobfuscate(&claims.sub).map_err(|_| AppError::Unauthorized)?;
    state.wallet_service.get_tx_history(user_id).await
        .map(Json)
        .map_err(|e| {
...
pub async fn eth_withdraw(
    ...
    let user_id = deobfuscate(&claims.sub).map_err(|_| AppError::Unauthorized)?;
    state.wallet_service.eth_withdraw(user_id, req).await
        .map(Json)
        .map_err(|e| {
