...
pub async fn list_products(
    ...
    state.product_service.list_products(query).await
        .map(Json)
        .map_err(|e| {
...
pub async fn get_product(
    ...
    state.product_service.get_product(product_id).await
        .map(Json)
        .map_err(|e| {
...
pub async fn create_product(
    ...
    state.product_service.create_product(seller_id, req).await
...
pub async fn update_product_status(
    ...
    state.product_service.update_product_state(seller_id, product_id, req).await
        .map(|_| StatusCode::OK)
        .map_err(|e| {
            tracing::error!("update_product_status error: {:?}", e);
            match e {
                crate::service::product_service::ProductServiceError::Forbidden => AppError::Forbidden,
                crate::service::product_service::ProductServiceError::NotFound => AppError::NotFound("Not found".to_string()),
                _ => AppError::Internal,
            }
        })
}
...
pub async fn update_product(
    ...
    state.product_service.update_product(seller_id, product_id, req).await
        .map(Json)
        .map_err(|e| {
