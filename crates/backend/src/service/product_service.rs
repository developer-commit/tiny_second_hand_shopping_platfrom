...
impl ProductServiceTrait for ProductService {
    ...
    async fn list_products(
        ...
        if let Some(uid) = &query.owner_uid {
            ...
            let seller_id = deobfuscate(uid)
                .map_err(|_| ProductServiceError::Internal("Invalid owner_uid".to_string()))?;
            filter = filter.filter(product::Column::SellerId.eq(seller_id));
        }
    ...
    async fn get_product(&self, product_id: i64) -> Result<ItemDetailRes, ProductServiceError> {
        ...
        p.into_dto(
            images,
            tags,
            Some(seller.trust_score.to_f64().unwrap_or(0.0)),
        )
        .map_err(|e| ProductServiceError::Internal(e.to_string()))
    }
    ...
    async fn update_product(
        ...
        if let Some(h) = req.heading {
            active_p.title = Set(h);
        }
        if let Some(d) = req.detail_body {
            active_p.description = Set(d);
        }
        if let Some(pr) = req.asking_price {
            let dec = Decimal::from_f64(pr).unwrap_or(Decimal::new(0, 0));
            active_p.price = Set(dec);
        }
        if let Some(c) = req.group_category {
            active_p.category = Set(c);
        }
        active_p.updated_at = Set(chrono::Utc::now().into());

        let updated_p = active_p
            .update(&self.db)
            .await
            .map_err(|e| ProductServiceError::Internal(e.to_string()))?;
