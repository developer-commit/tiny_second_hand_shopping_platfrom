...
impl EscrowServiceTrait for EthEscrowService {
    async fn initiate(
        ...
        use crate::db::entity::{product, wallet, escrow_trade};
        ...
        let seller_addr: Address = seller_address.parse().map_err(|_| EscrowServiceError::InvalidWalletAddress)?;
        ...
        let res: SafeTradeStatusRes = inserted_escrow.try_into().map_err(|e| EscrowServiceError::Internal(format!("DTO: {:?}", e)))?;
    ...
    async fn deposit(
        ...
        let seller_addr: Address = seller_wallet.eth_address.parse()
        ...
        let local_wallet = dummy_privkey.parse::<LocalWallet>()
            .map_err(|_| EscrowServiceError::Internal("Invalid private key".to_string()))?
            .with_chain_id(self.blockchain_manager.chain_id());
        let client = Arc::new(SignerMiddleware::new(self.blockchain_manager.provider().clone(), local_wallet));
        
        let contract = Escrow::new(self.contract_address, client);
        
        let call = contract.deposit(U256::from(trade_id), seller_addr).value(amount_u256);
        ...
        let tx_hash_str = match receipt {
            Some(r) => {
                if r.status != Some(U64::from(1)) {
                    return Err(EscrowServiceError::ContractRevert("Transaction failed on-chain".to_string()));
                }
                format!("{:?}", r.transaction_hash)
            },
            None => return Err(EscrowServiceError::Internal("No transaction receipt".to_string())),
        };
    ...
    async fn confirm(
        ...
        // INTERACT WITH SMART CONTRACT / EVM
        let admin_privkey = std::env::var("MASTER_WALLET_KEY")
        ...
        let local_wallet = admin_privkey.parse::<LocalWallet>()
        ...
        let tx_hash_str = match receipt {
            Some(r) => {
                if r.status != Some(U64::from(1)) {
                    return Err(EscrowServiceError::ContractRevert("Release transaction failed on-chain".to_string()));
                }
                format!("{:?}", r.transaction_hash)
            },
            None => return Err(EscrowServiceError::Internal("No transaction receipt".to_string())),
        ...
        new_tx.insert(&txn).await.map_err(|e| EscrowServiceError::Internal(e.to_string()))?;
    ...
    async fn dispute(
        ...
        let admin_privkey = std::env::var("MASTER_WALLET_KEY").unwrap_or_else(|_| "0xac0974bec39a17e36ba4a6b4d238ff944bacb478cbed5efcae784d7bf4f2ff80".to_string());
        let admin_privkey = admin_privkey.trim_start_matches("0x");
        let local_wallet = admin_privkey.parse::<LocalWallet>()
            .map_err(|_| EscrowServiceError::Internal("Invalid admin private key".to_string()))?
            .with_chain_id(self.blockchain_manager.chain_id());
        ...
        let pending_tx = call.send().await.map_err(|e| EscrowServiceError::RpcError(e.to_string()))?;
        let receipt = tokio::time::timeout(std::time::Duration::from_secs(30), pending_tx)
            .await
            .map_err(|_| EscrowServiceError::RpcError("Transaction receipt timeout".to_string()))?
        ...
        match receipt {
            Some(r) => {
                if r.status != Some(U64::from(1)) {
                    return Err(EscrowServiceError::ContractRevert("Dispute transaction failed on-chain".to_string()));
                }
            },
            None => return Err(EscrowServiceError::Internal("No transaction receipt".to_string())),
        }
    ...
    async fn get_trade(
        ...
        let res: SafeTradeStatusRes = trade.clone().try_into().map_err(|e| EscrowServiceError::Internal(format!("DTO: {:?}", e)))?;
