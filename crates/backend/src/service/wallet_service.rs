...
impl WalletServiceTrait for WalletService {
    ...
    async fn get_wallet_state(&self, user_id: i64) -> Result<WalletStateRes, WalletServiceError> {
        ...
        let info = WalletDomain::get_balance_info(
            &self.db,
        ...
        Ok(wallet.into_dto(
            info.available_eth_balance,
            info.display_locked_amount,
            total_eth,
        ))
    }
    ...
    async fn eth_withdraw(
        ...
        self.auth_service
            .verify_otp(user_id, &req.otp_token)
            .await
            .map_err(|_| WalletServiceError::OtpFailed)?;

        if !self
            .wallet_port
            .validate_address(&req.destination_eth)
            .await
            .unwrap_or(false)
        {
        ...
        let fee_u256 = self
            .wallet_port
            .estimate_fee(amount_u256)
            .await
            .map_err(|e| WalletServiceError::Internal(e.to_string()))?;

        let fee_eth_str =
            ethers::utils::format_units(fee_u256, "ether").unwrap_or_else(|_| "0".to_string());
        ...
        WalletDomain::verify_sufficient_funds(
            &txn,
            self.wallet_port.clone(),
            user_id,
            "ETH",
        ...
        Ok(TxHistoryItemRes {
            tx_uid: crate::utils::security::obfuscate(inserted.id)
                .map_err(|_| WalletServiceError::Internal("ID error".to_string()))?,
